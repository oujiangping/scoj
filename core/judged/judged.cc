...
void init_mysql_conf() {
	...
	if(fp!=NULL){
		...
		sprintf(query,"SELECT solutionId FROM solution WHERE programLan in (%s) and result<2 and MOD(solutionId,%d)=%d ORDER BY result ASC,solutionId ASC limit %d",oj_lang_set,oj_tot,oj_mod,max_running*2);
...
bool _check_out_mysql(int solution_id,int result){
	...
	sprintf(sql,"UPDATE solution SET result=%d,runTime=0,memory=0,judgeTime=NOW() WHERE solutionId=%d and result<2 LIMIT 1"
			,result,solution_id);
	if (mysql_real_query(conn,sql,strlen(sql))){
