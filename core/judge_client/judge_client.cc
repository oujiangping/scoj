...
void _update_solution_mysql(int solution_id, int result, int time, int memory,
        ...
        if(oi_mode){
                   sprintf(
                        sql,
                        "UPDATE solution SET result=%d,runTime=%d,memory=%d,judgeTime=NOW() WHERE solutionId=%d LIMIT %d",
                        result, time, memory,solution_id, 1);
        }else{
                        sprintf(
                        sql,
                        "UPDATE solution SET result=%d,runTime=%d,memory=%d,judgeTime=NOW() WHERE solutionId=%d LIMIT %d",
        ...
        if (sim) {
                sprintf(
                                sql,
                                "insert into sim(solutionId,simSolutionId,sim) values(%d,%d,%d) on duplicate key update  simSolutionId=%d,sim=%d",
...
void _addceinfo_mysql(int solution_id) {
        ...
        snprintf(sql, (1 << 16) - 1,
                        "DELETE FROM compile_info WHERE solutionId=%d", solution_id);
...
void _addreinfo_mysql(int solution_id,const char * filename) {
        ...
        snprintf(sql, (1 << 16) - 1,
                        "DELETE FROM runtime_info WHERE solutionId=%d", solution_id);
        mysql_real_query(conn, sql, strlen(sql));
        rend = reinfo;
...
void _update_user_mysql(int& user_id) {
        ...
        sprintf(
                        sql,
                        "UPDATE `user_state` SET `solved`=(SELECT count(DISTINCT `problemId`) FROM `solution` WHERE `userId`=%d AND `result`=\'4\') WHERE `userId`=%d",
                        user_id, user_id);
        if (mysql_real_query(conn, sql, strlen(sql)))
                write_log(mysql_error(conn));
        sprintf(
                        sql,
                        "UPDATE `user_state` SET `submit`=(SELECT count(*) FROM `solution` WHERE `userId`=%d) WHERE `userId`=%d",
                        user_id, user_id);
        if (mysql_real_query(conn, sql, strlen(sql)))
                write_log(mysql_error(conn));
}
void _update_user_http(int& user_id) {
        ...
        const char  * cmd=" wget --post-data=\"updateuser=1&user_id=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/admin/problem_judge.php\"";
...
void _update_problem_mysql(int p_id) {
        ...
        sprintf(
                        sql,
                        "UPDATE `problem` SET `accepted`=(SELECT count(*) FROM `solution` WHERE `problemId`=\'%d\' AND `result`=\'4\') WHERE `problemId`=\'%d\'",
                        p_id, p_id);
        if (mysql_real_query(conn, sql, strlen(sql)))
                write_log(mysql_error(conn));
        sprintf(
                        sql,
                        "UPDATE `problem` SET `submit`=(SELECT count(*) FROM `solution` WHERE `problemId`=\'%d\') WHERE `problemId`=\'%d\'",
                        p_id, p_id);
        if (mysql_real_query(conn, sql, strlen(sql)))
                write_log(mysql_error(conn));
}
...
void _get_solution_mysql(int solution_id, char * work_dir, int lang) {
        ...
        MYSQL_RES *res;
        MYSQL_ROW row;
        sprintf(sql, "SELECT code FROM solution_code WHERE solutionId=%d",
                        solution_id);
        mysql_real_query(conn, sql, strlen(sql));
...
void _get_custominput_mysql(int solution_id, char * work_dir) {
        ...
        MYSQL_RES *res;
        MYSQL_ROW row;
        sprintf(sql, "SELECT inputText FROM custom_input WHERE solutionId=%d",
                        solution_id);
        mysql_real_query(conn, sql, strlen(sql));
        res = mysql_store_result(conn);
        row = mysql_fetch_row(res);
...
void _get_solution_info_mysql(int solution_id, int & p_id, int& user_id, int & lang) {
        ...
        sprintf(
                        sql,
                        "SELECT problemId, userId, programLan FROM solution where solutionId=%d",
                        solution_id);
        //printf("%s\n",sql);
        mysql_real_query(conn, sql, strlen(sql));
        res = mysql_store_result(conn);
...
void _get_solution_info_http(int solution_id, int & p_id, int& user_id, int & lang) {
...
void get_solution_info(int solution_id, int & p_id, int& user_id, int & lang) {
...
void _get_problem_info_mysql(int p_id, int & time_lmt, int & mem_lmt, int & isspj) {
        ...
        sprintf(
                        sql,
                        "SELECT timeLimit,memoryLimit FROM problem where problemId=%d",
                        p_id);
        mysql_real_query(conn, sql, strlen(sql));
        res = mysql_store_result(conn);
        row = mysql_fetch_row(res);
...
//jvm gc ask VM before need,so used kernel page fault times and page size
                ...
                if (topmemory > mem_lmt * STD_MB) {
                  ...
                  //sig = status >> 8;/*status >> 8 Ã¥Â·Â®Ã¤Â¸ÂÃ¥Â¤Å¡Ã¦ËÂ¯EXITCODE*/
...
int main(int argc, char** argv) {
    ...
    if(http_judge)
        ...
        if (p_id>0&&(dp = opendir(fullpath)) == NULL) {
                write_log("ereeeeeeeeeeeeeeeeeeeeeeeee    No such dir:%s!\n", fullpath);
